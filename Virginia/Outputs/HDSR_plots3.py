...
for elect in range(1):
    ...
    fig, ax = plt.subplots()
    draw_plot(a[:1000000,:],-2,'r',None)
    draw_plot(a[:5000000,:],0,'y',None)
    draw_plot(a[:10000000,:],2,'b',None)

    plt.plot([],[],color='r',label='1M Steps')
    plt.plot([],[],color='y',label='5M Steps')
    plt.plot([],[],color='b',label='10M Steps')
    ...
    plt.savefig("./Plots/HDSR2/FLIPlength_compare.png")
    fig = plt.gcf()
    fig.set_size_inches((12,8), forward=False)
    fig.savefig("./Plots/HDSR2/FLIPlength_compare2.png", dpi=1000)
    plt.close()
